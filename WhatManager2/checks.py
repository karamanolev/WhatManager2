        ...
        for instance in replica_set.transinstance_set.all():
            ...
            for hash, m_torrent in i_m_torrents.items():
                ...
                if hash in m_torrents:
                    warnings.append(u'{0} is already in another instance of the same replica set: {1}'.format(m_torrent, m_torrents[hash].instance))
                # Check if the instance has the torrent
                if not hash in i_t_torrents:
                    errors.append(u'{0} is in DB, but not in Transmission at instance {1}'.format(m_torrent, instance))
                ...
                if replica_set.is_master:
                    ...
                    if not any('.torrent' in f for f in files_in_dir):
                        errors.append(u'Missing .torrent file for {0} at {1}'.format(m_torrent, instance))
                    if not any('ReleaseInfo2.txt' == f for f in files_in_dir):
                        errors.append(u'Missing ReleaseInfo2.txt for {0} at {1}'.format(m_torrent, instance))

            for hash, t_torrent in i_t_torrents.items():
                # Check if the database has the torrent
                if not hash in i_m_torrents:
                    errors.append(u'{0} is in Transmission, but not in DB at instance {1}'.format(t_torrent, instance))
